...
import {PATHS,TRAITS,COMBOS,RELICS,CONSUMABLES,ENEMIES,CHAPTERS,EVENTS,DIFFICULTIES,xpCost,seeded,hashSeed} from './data.js';
...
import {revisedBalance,phase3Balance,dashCooldown,thunderCost,sigilGrowth,sigilArming,sigilTargets,stationaryActive,puppetReady} from './balance.js';
...
import {STARTER_EXTRAS} from './stat-sources.js';
...
export class Battle {
 constructor(options={},emit=()=>{}){
  ...
  this.player={x:WORLD.w/2,y:WORLD.h/2,hp:100,maxHp:100,shield:0,maxShield:0,mana:0,reserve:this.difficulty.reserve,maxReserve:this.difficulty.reserve,invuln:0,dash:0,dashCD:0,lastHit:-100,lastDash:-100,lastKill:-100,dx:0,dy:1,moving:false,buffs:{},revived:false};
  ...
  if(this.mode==='training'){this.chapter=0;this.level=meta.includes(PATHS[this.path].id+'-meta-5')?72:36;this.gold=500;for(let j=0;j<12;j++)this.traits[PATHS[this.path].id+'-'+j]=1;this.recalc();}
 ...
 recalc(){
  const old=this.stats||{},s={};sumMods(s,PATHS[this.path].perk);if(this.rulesVersion>=2)sumMods(s,STARTER_EXTRAS[this.path]);
  sumMods(s,cultivationMods(this.options,this.path));
  for(const [id,count]of Object.entries(this.traits)){const t=TRAITS.find(x=>x.id===id);if(t)for(let i=0;i<count;i++)sumMods(s,t.mods);}
  ...
  if(this.player){const p=this.player;const bonus=(this.options.metaRulesVersion<2?this.options.meta||[]:[]).filter(x=>x.endsWith('-meta-2')).length*2;
   p.maxHp=100+(s.hp||0)+bonus;p.maxShield=s.shield||0;p.maxReserve=this.difficulty.reserve+(s.reserve||0);
 ...
 syncSwords(){const n=Math.min(72,this.level);while(this.swords.length<n)this.swords.push({x:this.player.x,y:this.player.y,px:this.player.x,py:this.player.y,a:0,cool:this.swords.length*.012,target:null,hits:[],life:0});}
 ...
 addXP(value){
  ...
  while(this.xp>=xpCost(this.level)&&this.level<160){this.xp-=xpCost(this.level);this.level++;gained++;if(this.level%4===0)this.queuedChoices++;}
 ...
 dash(){
  ...
  if(this.rulesVersion>=2&&this.path===4&&this.time-this.lastSigil>=6-(s.metaSigilCooldown||0)){this.lastSigil=this.time;this.cue('sigil-place','闪避起点 · 留符','#ffbd78');const r=(s.dashFire?100:85)*(1+(s.metaSigilRadius||0)),modern=revisedBalance(this);const zone={x:modern?clamp(p.x,r,WORLD.w-r):p.x,y:modern?clamp(p.y,r,WORLD.h-r):p.y,r,ttl:modern?2.75:1.15,warn:modern?2.4:s.dashFire?.55:.8,fuseDuration:modern?2.4:s.dashFire?.55:.8,...(modern?{arming:sigilArming(this)}:{}),kind:'sigil',friendly:true,sigil:true,damage:(55+this.level*1.2)*(s.dashFire?1.35:1)*(1+(s.burnDamage||0))*sigilGrowth(this)};
 ...
 thunder(){
  ...
  p.mana=0;p.reserve-=cost;this.thunders++;this.thunderTime=this.time;this.toast('辟邪神雷 · 已施放，耗用 '+cost+' 雷源');
 ...
 sigilReadiness(){const left=Math.max(0,6-(this.stats.metaSigilCooldown||0)-(this.time-this.lastSigil));const pending=this.zones.find(z=>z.twin&&!z.pair&&!z.fired);return {left,label:pending?(left>0?'次符 '+left.toFixed(1)+'s':'可放第二符'):left>0?'留符 '+left.toFixed(1)+'s':'起点留符已就绪'};}
 consume(index){
 ...
 swordDamage(e){const s=this.stats,p=this.player;let m=1+(s.damage||0)+Math.floor(Math.min(72,this.level)/12)*(s.swordScale||0);
  ...
  if(this.time-this.thunderTime<5)m+=(s.afterThunder||0)+(this.rulesVersion>=2&&this.path===1?.25:0);if(p.mana>=100)m+=s.fullMana||0;
 ...
 updateAllies(dt){
  ...
  this.insectPursuits=[];for(let i=0;i<bugs;i++){const b=this.insects[i];b.cool-=dt;let e=this.allyTarget(b.x,b.y,250*(1+(s.metaInsectRange||0)),{kind:'insect',index:i});if(!e)e=this.allyTarget(p.x,p.y,350*(1+(s.metaInsectRange||0)),{kind:'insect',index:i});let tx=p.x+Math.cos(i*2.4+this.time)*55,ty=p.y+Math.sin(i*2.4+this.time)*55;if(e){this.insectPursuits.push(e.id);tx=e.x+Math.cos(i*2.4)*12;ty=e.y+Math.sin(i*2.4)*10;}
   const d=Math.hypot(tx-b.x,ty-b.y)||1,v=Math.min(d,220*(1+(s.insectSpeed||0))*dt);b.x+=(tx-b.x)/d*v;b.y+=(ty-b.y)/d*v;
   if(e&&d<e.r+12&&b.cool<=0){let dmg=(8+this.level*.18)*(this.rulesVersion>=2&&this.path===3?.68*(1+Math.floor(Math.min(72,this.level)/12)*.2):1)*(1+(s.insectDamage||0));if(e.boss||e.elite)dmg*=1+(s.insectBoss||0);if(e.mark)dmg*=1+(s.insectMark||0);if(this.comboSet.has('swordInsect')&&this.rng()<.2)dmg*=1.8;const hp=e.hp;this.hit(e,dmg,'insect');if(this.modeState!==phase||this.finished)return;if(e.hp<hp){if(this.rulesVersion>=2&&this.path===3){e.gnaw=Math.min(.12,(e.gnawUntil>this.time?e.gnaw||0:0)+.02);e.gnawUntil=this.time+3+(s.metaGnawDuration||0);if(e.gnaw>=.12)this.cue('gnaw','噬甲已满 · 受伤 +12%','#dab4ff');}e.break=Math.min(.5,e.break+(s.insectBreak||0));if(s.insectMana&&this.rng()<s.insectMana)p.mana=Math.min(100,p.mana+1);}b.cool=.65/(1+(s.insectHaste||0));}
  }
 }
 update(dt){
 ...
 chooseEvent(effect){
  ...
  if(effect==='gold')this.gold+=45;if(effect==='heal')p.hp=Math.min(p.maxHp,p.hp+p.maxHp*.2);if(effect==='xp')this.addXP(xpCost(this.level)*.7);if(effect==='shield')p.shield+=25;
