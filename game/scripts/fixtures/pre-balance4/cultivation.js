import {stationaryActive} from './balance.js';
...
const changes={
 'thunder-meta-3':{desc:'雷法在原本允许补雷源的节点休整额外恢复 5 雷源，合计最多 20，受储量上限限制；宝光、幻境、章末和终章不补，每节点仅一次。',mods:{metaReserveRest:5}},
...
export function cultivationDetail(m,b){
 ...
 'talisman-meta-3':()=>m.mods.metaSigilArming?`本局普通布符 ${n(Math.max(.1,.35-(s.metaSigilArming||0)))} 秒、遁后留符 ${n(Math.max(.1,.2-(s.metaSigilArming||0)))} 秒；未决符按放置时快照。`:`本局留符独立冷却 ${n(6-(s.metaSigilCooldown||0))} 秒。`,
