import {SEGMENT_VERSION,CHALLENGE_VERSION,segmented,challenged,appendChapterSplit,prepareChallenge,chooseChallenge,commitChallenge,challengeBlocks,validateExperienceSnapshot} from './experience-rules.js';
...
import {treasureStepSeconds,illusionFirstWait,validatePacing} from './pacing-rules.js';
import {ECONOMY_VERSION,endingCleanup,revisedEconomy,emptyEconomy,variedCoreOffers,rerollPurchaseReadiness,buyReroll,validatePhase3} from './economy-rules.js';
...
import {ENCOUNTER_VERSION,latePressure,mechanismPressure,mechanismProfile,mechanismLimit,mechanismDeadline,mechanismClearing,clearingCheckpoint,revisedDirector,finitePursuit,ordinaryCount,ordinaryLimit,clearing,beginDirectorNode,updatePursuit,budgetSpawns} from './director-rules.js';
...
import {BALANCE_VERSION,reserveRestoration,phase3Balance,puppetReady,sigilPairFuse} from './balance.js';
...
export class Expedition extends Battle{
 ...
 playCinematic(id,after='rest',replay=false){this.cinematic={id,panel:0,after,replay,returnScene:this.scene,returnState:this.modeState};if(!replay&&persistentStory(this)&&this.options.settings?.skipSeenCinematics&&this.cinematicHistory.includes(id)){this.advanceCinematic(true);return;}this.showScene('cinematic','rest');if(!replay)this.checkpoint();}
 ...
 beginNode(){
  ...
  if(['serpent','ghost','puppet','xuangu'].includes(k)){super.spawnBoss();const hp={serpent:9000,ghost:19000,puppet:80000,xuangu:100000}[k];this.boss.hp=this.boss.maxHp=hp*this.difficulty.hp;}
  else if(k==='swarm')this.boss={type:'swarm',name:e.name,hp:75,maxHp:75,timer:75};
  ...
  else if(k==='illusion')this.objective={...(this.options.encounterVersion>=7?{returnProtectedUntil:0}:{}),...(clearingCheckpoint(this)?{clearingSaved:false}:{}),kind:k,done:0,next:illusionFirstWait(this),cool:0,gates:[{x:320,y:250},{x:640,y:250},{x:960,y:250}],trueGate:this.chapter%3};
  else if(k==='ice')this.objective={...(clearingCheckpoint(this)?{clearingSaved:false}:{}),kind:k,x:640,y:260,r:40,exit:{x:1130,y:640,r:80}};
  else if(k==='defend')this.objective={kind:k,x:640,y:360,hp:360+this.chapter*35,maxHp:360+this.chapter*35,invuln:0,done:0};
 ...
 spawn(type,opts={}){
  ...
  e.speed=opts.speed??base.speed*(1+this.chapter*.035);e.dmg=opts.dmg??base.damage*(1+this.chapter*.13)*this.difficulty.damage;
  if(this.objective?.kind==='defend'&&!e.boss&&e.id%3!==0)e.focusObject=this.objective;return e;
 }
 ...
 updateDirector(dt){
  ...
  if(k==='illusion'){q.cool=Math.max(0,q.cool-dt);if(this.waveTime>=q.next&&q.cool===0){const at=q.gates.findIndex(g=>distance(p,g)<65);if(at>=0){if(this.options.encounterVersion>=7){q.returnProtectedUntil=this.time+.45;this.cue('illusion-return','落点保护 · 0.45秒','#a9e6ef',.45);}if(at===q.trueGate){q.done++;q.next=Math.max(q.next+10,this.waveTime+2);q.trueGate=(q.trueGate+1)%3;this.player.x=640;this.player.y=560;this.toast('神识辨真 · '+q.done+' / 3');this.cue('illusion-true','已辨真 '+q.done+' / 3 · 寻找下一道双环','#c7edd0',3);if(q.done===3){this.completeNode();return;}}else{q.cool=2;this.player.y=560;this.toast('虚妄之门 · 观察「真」字与双环');this.cue('illusion-false','此门为虚 · 已退回，观察真字与双环','#ecc3ac',2);}}}if(!mechanismPressure(this))return;}
  if(k==='ice'){q.r=Math.min(380,40+this.waveTime*8.5);if(distance(p,q)<q.r)this.hitPlayer((14+this.chapter*2)*this.difficulty.damage,true,{name:'乾蓝冰焰',x:q.x,y:q.y,kind:'险地'});if(this.waveTime>=40&&distance(p,q.exit)<q.exit.r){this.completeNode();return;}if(!mechanismPressure(this))return;}
  if(k==='defend'){q.invuln=Math.max(0,q.invuln-dt);if(this.encounter.variant===2){q.x=640+Math.sin(this.waveTime*.045)*220;q.y=360+Math.cos(this.waveTime*.045)*80;}if(this.waveTime>=65){this.completeNode();return;}}
  ...
  if(k==='survival'&&this.waveTime>=65){this.completeNode();return;}
  if(k==='swarm'){this.boss.timer=Math.max(0,75-this.waveTime);this.boss.hp=this.boss.timer;if(this.waveTime>=75){this.completeNode();return;}}
  ...
  const boss=['serpent','ghost','puppet','xuangu'].includes(k),rate=mechanismPressure(this)?mechanismProfile(this).rates[k]:(boss?.8:k==='swarm'?6:2.7+this.chapter*.45)*(latePressure(this)&&!boss?1.35:1),pressure=this.waveTime>=40?1.3:this.waveTime>=20?1.15:1;
 ...
 completeNode(){
  ...
  this.rest={selectionMode:'draft',...(ending?{ending:true}:{}),eventApplied:ending,coreApplied:false,coreSelection:null,node:n.id,last,event:ending?null:this.makeEvent(),eventDone:ending,cashReserve,bonus,riskGold,xp:n.xp,coreOffers:last&&[0,2].includes(this.chapter)?this.makeCoreOffers():[],coreDone:!last||![0,2].includes(this.chapter),shop:(!last&&this.wave===1&&this.chapter!==3),routeDone:last||this.routes[this.chapter][this.wave+1].selected!==null};
 ...
 singleEffectText(effect){const gold=this.rest?.cashReserve||0,p=this.player,n=v=>Number(v.toFixed(1));return {xp:'参悟心得，获得 1 次悟道重选',riskTrait:'舍去 12% 当前生命，获得 2 次悟道重选',shield:'获得 20 额外护盾',gold:'领取 '+gold+' 灵石',smallGold:'领取 '+gold+' 灵石',riskGold:'舍去 8% 当前生命，另恢复 25 灵力并领取 '+gold+' 灵石',heal:'恢复 18% 最大生命（当前可恢复 '+n(Math.min(p.maxHp-p.hp,p.maxHp*.18))+' 生命）',mana:'恢复 40 灵力，不补雷源（当前可恢复 '+n(Math.min(100-p.mana,40))+' 灵力）',reroll:'获得 1 次悟道重选',escape:'恢复遁步并获得 15 额外护盾',riskRelic:'舍去 10% 当前生命，获得 1 次悟道重选与 25 灵力',buyHeal:'支付 25 灵石，恢复 45% 最大生命（当前可恢复 '+n(Math.min(p.maxHp-p.hp,p.maxHp*.45))+' 生命）',buyGuard:'支付 15 灵石，取得金光符（需药囊空位）',medicine:'取得回元药包（需药囊空位）',sparePuppet:'取得备用傀儡（需药囊空位）',fireItem:'取得火球符（需药囊空位）',insectItem:'取得灵虫诱饵（需药囊空位）',magnetItem:'取得收摄符（需药囊空位）',skip:'整理预留物资，领取 '+gold+' 灵石'}[effect]||'继续前行';}
 applySingle(effect){const p=this.player,cash=this.rest.cashReserve,items={medicine:'heal',sparePuppet:'puppet',fireItem:'fire',insectItem:'insect',magnetItem:'magnet',buyGuard:'guard'};
  ...
  if(['gold','smallGold','riskGold','skip'].includes(effect))this.gold+=cash;
 ...
 static restore(data,emit=()=>{}){
  validateCommonSnapshot(data);validateWorldSnapshot(data.expedition);
