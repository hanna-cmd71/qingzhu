 ...
 */
import {thunderBaseCost,traitMods} from './balance.js';
import {TRAITS,xpCost} from './data.js';
export const GROWTH_VERSION=5;
export const revisedGrowth=b=>b.rulesVersion===3&&b.options.growthVersion>=2;
export const phase5Growth=b=>b.rulesVersion===3&&b.options.growthVersion===5;
...
// Growth 5 (round-2 batch B-6): 1500 base XP moves from chapter 5 into chapters 3/4; chapters 1/2/6 keep growth 4; total remains 11960.
// The isolated experiment simulator may overwrite GROWTH5 in-process; the shipped game uses these values. Growth 1–4 keep xpCost and their tables.
export const XP_V5={2:[600,650,700,750],3:[1000,1000],4:[1500,1550,1400,350],5:[400,200,250,150]};
export const GROWTH5={xp:XP_V5,quad:.030,ownGuarantee:false,noForeignKnock:true}; // ownGuarantee (own-path card in every offer) was tested and not adopted: it lowered thunder/talisman/guard clears under the reference pilot
// Growth 5 keeps the growth-4 goal award; only the XP layout, level cost and candidate guarantee change.
export const goalGrowth=b=>b.rulesVersion===3&&b.options.growthVersion>=4;
export const xpCostFor=(b,l)=>phase5Growth(b)?Math.round(8+2.4*l+GROWTH5.quad*l*l):xpCost(l);
...
export const growthNodeXP=(options,chapter,node,original)=>options.growthVersion===5?(GROWTH5.xp[chapter]?.[node]??original):options.growthVersion===4?(XP_V4[chapter]?.[node]??original):options.growthVersion===3?(XP_V3[chapter]?.[node]??original):original;
...
export function traitReadiness(t,b){
 ...
 const burning=(s.burn||0)>0,hasSigil=b.path===4&&b.rulesVersion>=2||s.dashFire>0,mods=traitMods(b,t);
 const reasonFor=key=>{
  if(key==='knock'&&b.path!==4&&phase5Growth(b)&&GROWTH5.noForeignKnock)return '震退只提供给符阵起手';
  ...
  if(key==='insects'&&activeBugs>=30&&(!s.insectGuard||Math.floor((bugs+(mods.insects||0))/4)===Math.floor(bugs/4)))return '协战虫数已达30只';
  ...
  if(key==='insectMark'&&phase5Growth(b)&&activeBugs<=0&&b.path!==1&&!s.mark&&!s.insectMark)return '需先有灵虫；无虫时只有雷法起手或已有雷印能用到印记';
 ...
 const reasons=Object.keys(mods).map(reasonFor),eligible=reasons.some(r=>!r);
...
export function growthChoices(b){
 ...
 // Own-path guarantee: growth 2–5 guarantee one own-path card on the first offer only. GROWTH5.ownGuarantee (every offer) was tested in the batch-B matrix and not adopted; the switch ships false, so growth 5 behaves like growth 2–4 here.
 if(!b.growth.firstOfferUsed||phase5Growth(b)&&GROWTH5.ownGuarantee){const own=pool.filter(t=>t.path===b.path);if(own.length)pick(own);}
