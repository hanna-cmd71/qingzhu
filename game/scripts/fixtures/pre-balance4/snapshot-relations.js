...
export function validateSnapshotRelations(data,battle){
 ...
 for(const z of ex.zones){if(z.arming!==undefined&&![2,3].includes(battle.options.balanceVersion))fail('布符与战斗平衡规则');coordinate(z,'场地位置与战场');bounds(z.ttl,-.051,12,'场地剩余时间');bounds(z.warn,-1e8,8,'场地预警时间');if(z.fuseDuration!==undefined&&z.warn>z.fuseDuration+EPS)fail('符阵剩余引信');if(z.line)coordinate(z.line,'火线位置与战场');}
}
