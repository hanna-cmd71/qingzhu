...
export function statSources(b){
 const traits={},equipment={};for(const [id,n] of Object.entries(b.traits))sum(traits,TRAITS.find(t=>t.id===id)?.mods,n);for(const id of b.relics)sum(equipment,RELICS.find(r=>r.id===id)?.mods);
 const oldHp=(b.options.metaRulesVersion<2?b.options.meta||[]:[]).filter(id=>id.endsWith('-meta-2')).length*2;
 const rows=[{name:'基础与起手',mods:{hp:100,reserve:b.difficulty.reserve,...PATHS[b.path].perk}},{name:'起手规则额外值',mods:b.rulesVersion>=2?STARTER_EXTRAS[b.path]:{}},{name:'出发修持',mods:{...cultivationMods(b.options,b.path),...(oldHp?{hp:oldHp}:{})}},{name:'悟道与章节预置',mods:traits},{name:'装备',mods:equipment}];
 ...
 derived.armor=(derived.armor||0)+Math.floor((b.stats.insects||0)/4)*(b.stats.insectGuard||0);derived.shield=(b.stats.puppets||0)*(b.stats.puppetShield||0);rows.push({name:'联动与数量换算',mods:derived});
