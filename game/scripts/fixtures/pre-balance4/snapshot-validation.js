...
export function validateCommonSnapshot(d){
 for(const key of ['segmentVersion','challengeVersion'])if(d?.options?.[key]!==undefined&&![0,1].includes(d.options[key]))fail('体验独立版本');
 ...
 if(d.options.growthVersion!==undefined&&![1,2,3,4].includes(d.options.growthVersion))fail('成长规则版本');
 ...
 if(d.options.balanceVersion!==undefined&&![1,2,3].includes(d.options.balanceVersion))fail('战斗平衡版本');
...
export function validateWorldSnapshot(ex){
 ...
 for(const z of ex.zones){object(z,'场地效果');for(const k of ['x','y','ttl','warn'])number(z[k],'场地效果状态');number(z.r,'场地范围',0,5000);number(z.damage,'场地伤害',0,POWER);
  ...
  if(z.arming!==undefined){number(z.arming,'布符剩余时间',0,.35);if(!z.sigil||z.twin||z.metaEcho||!z.friendly||z.fuseDuration!==2.4)fail('布符触发规则');}
