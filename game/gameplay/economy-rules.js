...
// Round-2 C-7: the three cash outcomes of a single event differ. Economy-4 rests store the split when the rest is created
// (gold = node reserve as before, 继续前行 = half, risk = double); a rest saved without the split keeps the old uniform amount.
export const splitCashRewards=b=>b.rulesVersion===3&&b.options.economyVersion===4;
export const cashOptionsFor=(b,cash)=>splitCashRewards(b)?{gold:cash,skip:Math.floor(cash/2),risk:cash*2}:null;
export const cashRewardFor=(rest,effect)=>{const o=rest?.cashOptions,cash=rest?.cashReserve||0;if(!o)return cash;return effect==='skip'?o.skip:effect==='riskGold'?o.risk:o.gold;};
...
export function validatePhase3(data,b){
 ...
 if(ex.rest?.cashOptions!==undefined){const o=ex.rest.cashOptions,cash=ex.rest.cashReserve;if(!splitCashRewards(b)||!o||typeof o!=='object'||Object.keys(o).sort().join()!=='gold,risk,skip'||!Number.isSafeInteger(cash)||o.gold!==cash||o.skip!==Math.floor(cash/2)||o.risk!==cash*2)fail('机缘灵石份额');}
}
